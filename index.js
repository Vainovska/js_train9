...
function findIndexByProperty(arr, name) {
  ...
  const isName = arr.findIndex((elem, index, arr) => elem === name);
  return isName;
...
function findLastEvenNumber(arr) {
  ...
  const evenNumber = arr.findLast((elem) => elem % 2 === 0);
  if (evenNumber) {
    return evenNumber;
  } else {
...
function getArrayKeys(arr) {
  ...
  const arrArray = Array.from(arrayKeys);
  return arrArray;
...
function getPositiveArrayValues(arr) {
  ...
  const interator = arr.values();
  const newArr = Array.from(interator);
  const polArr = newArr.filter((elem) => elem > 0);
  return polArr;
...
function removeAndDouble(arr, startIdx) {
  ...
  const doubl = arr.map((elem) => elem * 2);
  return doubl;
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const square = flatArr.map((elem) => elem * elem);
  const filter = square.filter((elem) => elem > threshold);
  return filter;
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const initVal = 0;
  const sumArr = last.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue,
    initVal
  );
  last.forEach((elem) => console.log(elem));
  return sumArr;
 ...
 *  n - кількіс ть разів, скільки кожне число повторюється в новому масиві.
...
function findLongestWord(arr) {
  ...
  const newArr = arr.reduce((longest, currentValue) => {
...
function findDuplicateElements(arr) {
  ...
  const newArr = arr.filter((elem, index, arr) => arr.indexOf(elem) !== index);
  return newArr;
...
function capitalizeWords(arr) {
  ...
  const newArr = arr.map((n) => `${n[0].toUpperCase()}${n.slice(1)}`);
  return newArr;
...
function calculateTotalPrice(arr) {
  ...
  const newArr = arr.reduce(
    (price1 = arr.price, price2 = arr.price) => price1 + price2,
    initVal
  );
...
function findLastNegativeNumberIndex(arr) {
  ...
  const newArr = arr.findLastIndex((elem) => elem < 0);
  return newArr;
